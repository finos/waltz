<script>
    ...
    import {formatOperation} from "./analytics-utils";
    ...
    $: call = (startDate && endDate)
        ? analyticsDashboardStore.findChangesByOperation(startDate, endDate)
        : null;
    $: loading = call ? $call.status === REMOTE_API_STATUS.LOADING : true;
    $: error = call && $call.error ? "Failed to load operations data" : null;
    ...
    function toChartData(data) {
        ...
        if (!data || Object.keys(data).length === 0) {
            return { title: "Changes by Operation Type", data: [] };
        }
...
<SubSection>
    <div slot="header">
        ...
        <span class="pull-right"><ChartInfo help="Change-log entries grouped by operation type. 'UNKNOWN' means entries recorded without an operation."/></span>
    </div>
    <div slot="content">
