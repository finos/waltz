<script>
    ...
    $: call = (startDate && endDate && period)
        ? analyticsDashboardStore.findOperationTrends(period, startDate, endDate)
        : null;
    $: loading = call ? $call.status === REMOTE_API_STATUS.LOADING : true;
    $: error = call && $call.error ? "Failed to load operation trends data" : null;
    ...
    function toChartData(data) {
        ...
        if (!data || Object.keys(data).length === 0) {
            return { title: "Operation Trends Over Time", operations: [], periods: [] };
        }
...
<SubSection>
    <div slot="header">
        ...
        <span class="pull-right"><ChartInfo help="Change-log entries per period, split by operation type, across the selected range."/></span>
    </div>
    <div slot="content">
