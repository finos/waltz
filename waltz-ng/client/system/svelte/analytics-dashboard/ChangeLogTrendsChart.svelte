<script>
    ...
    $: call = (startDate && endDate && period)
        ? analyticsDashboardStore.findChangeLogTrends(period, startDate, endDate)
        : null;
    ...
    function toChartData(data, period) {
        ...
        if (!response || !response.data || response.data.length === 0) {
            return { title: response?.title || "Change Log Trends", data: [] };
        } else {
            // Filter out zero values if all values are zero
            const hasNonZeroData = response.data.some(item => item.value > 0);
            return hasNonZeroData ? response : { title: response.title, data: [] };
        }
    }
...
<SubSection>
    <div slot="header">
        ...
        <span class="pull-right"><ChartInfo help="Number of change-log entries recorded per period in the selected range."/></span>
    </div>
    <div slot="content">
