<script>
    ...
    $: call = (startDate && endDate) ? analyticsDashboardStore.findActivityByHour(startDate, endDate) : null;
    $: loading = call ? $call.status === REMOTE_API_STATUS.LOADING : true;
    $: error = call && $call.error ? "Failed to load activity heatmap data" : null;
    $: chartData = toChartData(call ? $call.data : null);

    function toChartData(data) {
        // Check if data is empty
        if (!data || !Array.isArray(data)) {
            return { title: "Accesses by Hour of Day", data: [] };
        }
...
<SubSection>
    <div slot="header">
        ...
        <span class="pull-right"><ChartInfo help="Number of page hits for each hour of the day. This counts hits, not distinct users."/></span>
    </div>
    <div slot="content">
