<script>
    ...
    $: call = (startDate && endDate) ? analyticsDashboardStore.findTopPages(startDate, endDate, 15) : null;
    $: loading = call ? $call.status === REMOTE_API_STATUS.LOADING : true;
    $: error = call && $call.error ? "Failed to load top pages data" : null;
    $: chartData = toChartData(call ? $call.data : null);

    function toChartData(data) {
        // Check if data is empty or invalid
        if (!data || !Array.isArray(data) || data.length === 0) {
            return { title: "Top Pages by Access Count", data: [] };
        }
...
<SubSection>
    <div slot="header">
        ...
        <span class="pull-right"><ChartInfo help="The most-visited pages in the selected range, ranked by number of hits."/></span>
    </div>
    <div slot="content">
