...
package org.finos.waltz.jobs.generators.stress;
...
public class LogicalFlowStressGenerator {
    ...
    private void generateDataTypeMappings() {
        ...
        Set<LogicalFlowDecoratorRecord> decorators = flowIds.stream()
                ...
                .distinct()
                .map(t -> WaltzUtilities.mkLogicalFlowDecoratorRecord(t.v1, t.v2.entityReference().id(), PROVENANCE))
                .collect(toSet());
    ...
    private void generateFlows() {
        List<Long> appIds = WaltzUtilities.getActiveAppIds(dsl);
        ...
        Set<LogicalFlowRecord> flows = randomPick(appIds, HOW_MANY_FLOWS)
                ...
                .map(t -> WaltzUtilities.mkLogicalFlowRecord(t.v1, t.v2, PROVENANCE))
                .collect(toSet());
